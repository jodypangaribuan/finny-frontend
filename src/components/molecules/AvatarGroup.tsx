    ...
    return (
        <div className="flex -space-x-4 overflow-hidden">
            {displayUsers.map((user, i) => (
                <Avatar key={i} src={user.src} fallback={user.fallback} className="ring-2 ring-card" />
            ))}
            {remaining > 0 && (
                <div className="flex size-10 items-center justify-center rounded-full bg-muted-bg text-sm font-medium text-foreground ring-2 ring-card">
                    +{remaining}
                </div>
            )}
        </div>
